...
**TLS.** Settled in step 5: `std.crypto.tls.Client` with `Certificate.Bundle` system roots and explicit hostname verification. Reasons: zero dependencies, it matches the new `std.Io` reader/writer model, and it supports the TLS 1.2/1.3 suites Azure offers. A live handshake against the real Event Hubs endpoint completed, so the mbedTLS fallback is no longer needed and the project does not vendor it.

Two properties of the standard library shape `transport.zig` and must survive any change to it. First, both the network read buffer and the network write buffer must hold `tls.Client.min_buffer_len` octets. A short read buffer trips an assert in `Client.init`, and a short write buffer panics inside `Io.Threaded` with `MSGSIZE` after a successful handshake, which no `catch` can see. Second, `Client.flush` encrypts into the buffer of the network writer and does not flush it (`crypto/tls/Client.zig:999`), so the octets stay in memory and the peer waits. `Transport.writer` therefore returns a writer that flushes the TLS writer and then the network writer, and every caller above the transport can treat `flush` as "the octets reached the peer".
...
- ~~**`std.crypto.tls.Client` interop is unproven against Event Hubs.**~~ Closed in step 5. A live TLS handshake, a SASL ANONYMOUS dialog, and the AMQP protocol header exchange all completed against the real endpoint. The mbedTLS fallback is dropped.
...
- **As built**: the fifth parameter is an `Options` struct, not a bare `tls_mode`, because the PLAIN mechanism needs the identity and the secret and there is no other place to put them. `Options` holds `tls`, `sasl`, `connect_timeout`, and `diagnostics`. The SASL dialog is also a free function, `performSasl(gpa, reader, writer, sasl, diagnostics)`, so a test drives it with `MockTransport` and no socket. The layer purity rule forbids the vendor name inside `src/amqp/`, so the live tests read `AMQP_LIVE_HOST`, `AMQP_LIVE_PORT`, `AMQP_LIVE_AUTHCID`, and `AMQP_LIVE_PASSWORD`; the README maps them to the fields of an Event Hubs connection string.
...
- **As built**: the third parameter is a `Stream` (a `*std.Io.Reader` and a `*std.Io.Writer`), not a `Transport`, because `MockTransport` is a different type from `Transport` and every test in this step drives the connection with no socket. `Stream.of(transport)` builds one from a `Transport`, and the connection does not own the transport, so the caller closes it after `deinit`. The demultiplexer copies the payload of each frame into the arena of the frame before it pushes the frame, because `Frame.payload` borrows the one read buffer that the task reuses. Section 2.4.1 fixes the maximum frame size at MIN-MAX-FRAME-SIZE until the two peers exchange their open frames, so a properties map that does not fit in 512 octets makes `open` return `error.FrameTooLarge`. The module names no error condition symbol: `close(condition)` takes one from the caller, and the demultiplexer answers a protocol error with a terminal state alone. `Options.close_timeout` bounds the close handshake, and a timeout still ends the connection so that no caller waits for a frame that cannot arrive. `Connection.failure()` gives the sticky reason as an `Error` with the condition symbol and the description text. Section 2.5.1 makes the outgoing channel of each peer independent, so the connection holds one table for each namespace: `registerChannel` writes the number of this peer, and the answering begin frame of the remote peer binds the number that it arrived on to the same session. The demultiplexer removes that binding itself when it routes the end frame of the remote peer, which is the point that note (1) of section 2.5.2 names. It must run in that task and in frame order, because the peer takes the same number for its next session and the answer for that session can be the very next frame.
...
- **As built**: `begin` takes three arguments, `Session.begin(connection, storage, options)`, and it reads the allocator and the `Io` from the connection, so that the two layers always share both. The second argument is new: the connection needs the memory of a channel queue to stay valid until `Connection.deinit` returns, so the caller owns it. `Storage.init(gpa, capacity)` allocates it, `begin` refuses a queue smaller than the incoming window, and the caller frees it after `Connection.deinit`. The order of the calls is therefore `Session.deinit`, `Connection.deinit`, `Storage.deinit`, which the defer statements of a caller give for free. The window arithmetic lives in a pure `Window` struct with its own unit tests. Section 2.5.6 says MAY twice, and this module decrements the incoming window on receipt, because that is what makes the replenishment rule mean something, and it holds the outgoing window still, because section 2.7.4 reads that field as a constant capacity. A `disposition` carries no handle in section 2.7.6, so it goes to every attached link, and the router copies the frame for all but the last one. An incoming `attach` routes by the link name of section 2.6.1, because the answering attach of the remote peer carries the handle of the remote peer and nothing that names the handle of this peer; every later frame of the link routes by that bound input handle. Section 2.5.1 gives a session two independent channel numbers, and the connection binds them, so the remote peer answers the begin on any outgoing channel of its own; both peers take the lowest free channel, which section 2.5.1 recommends. The router task of a session gives its outgoing number back when it reads the end frame of the remote peer, and not at `end` or `deinit`, because the peer reuses its own number at once and an application holds a dead session for as long as it wants. Unlike the connection, this module does name error condition symbols, because sections 2.7.4 and 2.8.17 make `amqp:session:unattached-handle` and `amqp:session:window-violation` the required answers to two peer errors. `attachLink(name, queue)` takes an output handle and holds the handle to link table, and it sends no frame, so the link layer of step 8 owns the attach handshake.
...
- **As built**: `link.zig` holds a `Link` value that a concrete link embeds, and not a base class. It owns the name, the output handle, the attach handshake, the detach handshake, the terminal state, and one router task; the concrete link gives it a `Handler` with two function pointers, and the router task passes every frame that is not an `attach` or a `detach` to that hook. Step 9 therefore adds a receiver without touching the handshake code. A remote `detach` that carries an error becomes a `link.Failure`, which holds the condition symbol and the description text, and which matches the shape of `Connection.failure` and `Session.failure`. `link.Storage` mirrors `session.Storage`: the owner of the link owns the memory of the frame queue, because the session holds a pointer to it until `Session.deinit` returns, so the call order is `Sender.deinit`, `Session.deinit`, `Connection.deinit`. `Deadline` reads the clock one time, because `std.Io.Event.waitTimeout` reports a spurious wakeup with the same error as a real timeout, so every wait of the link layer reads the clock itself before it gives up. `std.Io.Condition` has no wait with a timeout, so a `send` without credit starts one short task that broadcasts the condition when its deadline passes, and every waiter then reads its own deadline. Two changes reach into `session.zig`. First, `Session.send` now takes `state_mutex` and refuses a frame after the local end frame, because issue #36 recorded that an attach or a detach could otherwise cross the end frame, which section 2.5.5 forbids. Second, `Session.sendFirstTransfer` assigns the delivery id, because section 2.6 takes it "from a session scoped sequence" and two links on one session would otherwise collide; it takes the transfer id of section 2.5.6 for the value, so the id is unique and it follows the order of the frames on the wire without a second counter. `send` takes an options struct rather than the three positional arguments of this step, because Zig has no default argument and the specification gives `message-format` the default 0. The payload split sizes the transfer performative four times, for the first frame with and without `more` and for a later frame with and without `more`, because section 2.7.5 puts the delivery id, the delivery tag, and the message format on the first frame alone and a later frame therefore holds more message data.
...
- **As built**: `Receiver.attach(session, options)` mirrors `Sender.attach`, and `Options` nests `link.Options` under a field named `link`. The queue of deliveries holds `prefetch` messages, so the router task waits when no caller reads, and the credit top-up stops until a caller takes a delivery out. That wait is the backpressure of the link, and it moves back through the queue of the link and the queue of the session to the socket. `std.Io.Queue` has no get with a timeout, so `receive(timeout)` looks in the queue with a minimum of zero under a mutex and then waits on an `std.Io.Condition`, and one short task broadcasts that condition when the deadline passes, which is the pattern that step 8 built for the credit of the sender. The top-up fires when the deliveries that arrived take half of the window or more, and the arithmetic runs in 64 bits, so a prefetch of one tops up after one delivery. `StringFilter` is the generic filter helper: it takes a descriptor symbol and a string value, it allocates nothing, and it names no filter. A `Delivery` owns the joined octets and the decoded message, and one `deinit` frees both. Section 2.7.5 gives the receiver four rules that the code follows and that the tests hold: an unset `settled` on a later transfer frame of a delivery keeps the value of the frames before it, an aborted delivery reaches no caller and needs no disposition, a resumed delivery reaches no caller because this module sends no unsettled map, and `more` has the default false. A delivery that holds no message of section 3.2 draws a `rejected` disposition with `amqp:decode-error`, and a delivery past `max_message_size` ends the link with `amqp:link:message-size-exceeded`. Two things reach into `link.zig`. First, `Error.LinkRefused`, because section 2.6.3 makes a null local terminus the answer of a peer that refuses a link, and the local terminus of the remote peer is the `source` for a receiving endpoint; the sender still does not read its own `target` for that, which issue #38 records. Second, `Link.sendFrame` and `Link.sendFlow`, which give a concrete link one place to send a frame and to end the link when the session refuses it. The test doubles of step 8 moved to `src/amqp/test_peer.zig`, because the sender and the receiver drive the same scripted peer.
